(function() {
  ...
  function showExpeditionPopup() {
    ...
    info.innerHTML = `
      <strong>Normal</strong>: <br>(1 loot, ~1min)<br>
      <strong>Hardcore</strong>: <br>(2–4 loots, ~1hr)<br>
      <strong>Suicidal</strong>: <br>(5–10 loots, ~24hr)<br>
      <br>
      <em>Drop Rates:</em><br>
    ...
    normalBtn.addEventListener("click", function() {
      startExpedition("Normal", { duration: 1, lootMin: 1, lootMax: 1 });
    });
    ...
    hardcoreBtn.addEventListener("click", function() {
      startExpedition("Hardcore", { duration: 1, lootMin: 2, lootMax: 4 });
    });
    ...
    suicidalBtn.addEventListener("click", function() {
      startExpedition("Suicidal", { duration: 1, lootMin: 5, lootMax: 10 });
    });
  ...
  function finishExpedition(expeditionState) {
    ...
    if (success) {
      ...
      for (let i = 0; i < count; i++) {
        let customItem;
        // Ensure that the generated item is lootable (i.e. not a special item)
        do {
          customItem = generateCustomItem();
        } while(customItem.lootable === false);
