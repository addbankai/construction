  ...
  function showPvpModal(mode) {
    ...
    function simulateRound(){
      if (currentRound >= 8) {
        ...
        if(win) {
          if(mode === "item") {
            // Filter enemy equipment to include only lootable items.
            let enemyItems = enemy.equipment.filter(item => item && item.lootable !== false);
            ...
            if(typeof window.generateRandomItem === "function"){
              let lootReward;
              do {
                lootReward = window.generateRandomItem();
              } while(lootReward.lootable === false);
