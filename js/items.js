...
(function() {
  ...
  const itemTypes = [
    ...
    "Capsule", "PhoenixCapsule", "Enchantedtreasurebox", "Enchantedscroll"
  ];
  ...
  // Mapping for fixed descriptions for special items
  const specialDescriptions = {
    "Capsule": "Open to gain Hero",
    "PhoenixCapsule": "Open To gain Legendary Hero",
    "Enchantedtreasurebox": "Open to Gain legendary items",
    "Enchantedscroll": "Open to gain Skill Scroll"
  };
  ...
  function generateRandomItem() {
    // First, pick a random item type from the array.
    const randomType = randomElement(itemTypes);
    let item;
    
    // Check if the randomly chosen type is one of the special items.
    if (["Capsule", "PhoenixCapsule", "Enchantedtreasurebox", "Enchantedscroll"].includes(randomType)) {
      // For special items: no stats or power; fixed description; non-lootable.
      item = {
        ...
        render: function() {
          ...
          // For special items, a simple overlay.
          ...
          title.style.textShadow = "0 0 5px #FFD700";
          itemDiv.appendChild(title);
          return itemDiv;
        },
        getDescription: function() {
          // Return the fixed description for special items.
          return specialDescriptions[this.itemType];
        }
      };
    } else {
      // For regular items, generate full properties.
      const adjective = randomElement(adjectives);
      const material = randomElement(materials);
      const itemType = randomType;
      let rarity = randomElement(rarities);
      let image = itemImages[itemType] || "";
